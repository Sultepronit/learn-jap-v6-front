...
function getRowRefs (row: BTRow) {
    ...
    if (writ.innerHTML === "") { // row can be clonned
...
export default function fillRow(row: BTRow) {
    ...
    // genAnswerWritings(word) // DON'T CALL EVERY TIME!!!
    computeCommon(word)
    ...
    if (word.comp) {
        ...
        if (word.comp.common.writings.rare) {
            if (word.comp.common.writings.rare.isHtml) {
                row.refs.rareWrit.innerHTML = word.comp.common.writings.rare.value
            } else {
                row.refs.rareWrit.textContent = word.comp.common.writings.rare.value
            }
        } else {
    ...
    row.element.querySelector(".translation").textContent = word.card?.data.translation
